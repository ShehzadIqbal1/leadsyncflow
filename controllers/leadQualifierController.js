...
function getPktDateTime() {
  ...
  let pktTime = now.toLocaleTimeString("en-GB", { timeZone: "Asia/Karachi", hour12: false });
...
let updateLqStatus = asyncHandler(async function (req, res, next) {
  ...
  let lqStatus = String((req.body && req.body.lqStatus) || "").trim().toUpperCase();

  if (!isValidLqStatus(lqStatus)) {
    return next(httpError(statusCodes.BAD_REQUEST, "Invalid lqStatus"));
  }

  let lead = await Lead.findOne({ _id: leadId, stage: "LQ", assignedTo: req.user.id });
  if (!lead) return next(httpError(statusCodes.NOT_FOUND, "Lead not found / not assigned to you"));
...
let addComment = asyncHandler(async function (req, res, next) {
  ...
  if (!text) return next(httpError(statusCodes.BAD_REQUEST, "Comment text is required"));
  if (text.length > 1000) return next(httpError(statusCodes.BAD_REQUEST, "Comment too long"));
...
let assignToManager = asyncHandler(async function (req, res, next) {
  ...
  if (!managerId) return next(httpError(statusCodes.BAD_REQUEST, "managerId is required"));

  let lead = await Lead.findOne({ _id: leadId, stage: "LQ", assignedTo: req.user.id });
  if (!lead) return next(httpError(statusCodes.NOT_FOUND, "Lead not found / not assigned to you"));

  // verify manager exists and approved
  let manager = await User.findOne({ _id: managerId, role: "Manager", status: "APPROVED" }).select("_id role");
  if (!manager) return next(httpError(statusCodes.BAD_REQUEST, "Invalid managerId"));
